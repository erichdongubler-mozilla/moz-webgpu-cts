...
impl<'de> Deserialize<'de> for RunInfo {
        ...
        D: Deserializer<'de>,
    {
        ...
        let platform = match &*os {
            ...
            // TODO: Make sure we're not missing any important fields to validate for
            // Mac
            "mac" => Platform::MacOs,
            // TODO: Make sure we're not missing any important fields to validate for
            // Linux
...
pub(crate) enum TestExecutionResult {
    Complete {
        duration: TextExecutionDuration,
        #[serde(rename = "status")]
        outcome: TestOutcome,
    ...
    JobMaybeTimedOut {
        status: String,
        // TODO: It'd be nice to validate that `status` is still there (but set to
        // `""`) without keeping them around. 🤔 Custom `Deserialize` impl. or
        // a mono-state structure, I suppose?
        subtests: Vec<SubtestExecutionResult>,
    },
}
...
#[derive(Debug, Deserialize)]
#[serde(untagged)]
pub(crate) enum TextExecutionDuration {
    Valid(u64),
    /// Only a thing because of
    Invalid(f64),
}
